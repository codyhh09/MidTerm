...
{
public:
	CubemapEffect(ID3D11Device* device, const std::wstring& filename);
	~CubemapEffect();

	void SetWorldViewProj(CXMMATRIX M)                  { WorldViewProj->SetMatrix(reinterpret_cast<const float*>(&M)); }
	void SetCubeMap(ID3D11ShaderResourceView* cubemap)  { CubeMapSRV->SetResource(cubemap); }
...
public:
	...
	static CubemapEffect* CubemapFX;
};
#pragma endregion
