...
LPCTSTR WndClassName = L"MidTerm";
HWND hwnd = NULL;
HRESULT hr;
...
ID3D11Buffer* RunnerVertBuff;
ID3D11Buffer* RunnerIndexBuff;
std::vector<XMFLOAT3> RunnerVertPosArray;
std::vector<DWORD> RunnerVertIndexArray;
int RunnerSubsets = 0;
std::vector<int> RunnerSubsetIndexStart;
std::vector<int> RunnerSubsetTexture;
XMMATRIX RunnerWorld;
float RunnerBoundingSphere = 0.0f;
std::vector<XMFLOAT3> RunnerBoundingBoxVertPosArray;
std::vector<DWORD> RunnerBoundingBoxVertIndexArray;
XMVECTOR RunnerCenterOffset;
...
bool LoadObjModel(std::wstring filename, ID3D11Buffer** vertBuff, ID3D11Buffer** indexBuff, std::vector<int>& subsetIndexStart, std::vector<int>& subsetMaterialArray, std::vector<SurfaceMaterial>& material, int& subsetCount, bool isRHCoordSys, bool computeNormals, std::vector<XMFLOAT3>& vertPosArray, std::vector<DWORD>& vertIndexArray);
void StartTimer();
double GetTime();
double GetFrameTime();
...
void pickVector(float X, float Z, XMVECTOR& pickRayInWorldSpacePos, XMVECTOR& pickRayInWorldSpaceDir);
...
bool InitializeWindow(HINSTANCE hInstance, int ShowWnd, int width, int height, bool windowed){
	...
	hwnd = CreateWindowEx(NULL, WndClassName, L"MidTerm", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, width, height, NULL, NULL, hInstance, NULL);
...
bool InitScene()
{
	...
	hr = D3DX11CreateTextureFromFile(d3d11Device, L"skymap.dds", &loadSMInfo, 0, (ID3D11Resource**)&SMTexture, 0);
...
bool InitD2D_D3D101_DWrite(IDXGIAdapter1 *Adapter){
	...
	hr = DWriteFactory->CreateTextFormat(L"Script", NULL, DWRITE_FONT_WEIGHT_REGULAR, DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, 24.0f, L"en-us", &TextFormat);
...
void CleanUp(){
	...
	RunnerVertBuff->Release();
	RunnerIndexBuff->Release();
}
...
void pickVector(float X, float Z, XMVECTOR& pickRayInWorldSpacePos, XMVECTOR& pickRayInWorldSpaceDir)
{
	...
	//Transform 2D pick position on screen space to 3D ray in View space
	PRVecX = (((2.0f * X) / ClientWidth) - 1) / camProjection(0, 0);
	PRVecY = 1.0f;	//View space's Z direction ranges from 0 to 1, so we set 1 since the ray goes "into" the screen
	PRVecZ = -(((2.0f * Z) / ClientHeight) - 1) / camProjection(1, 1);
...
void DrawScene()
{
	...
	for (int i = 0; i < RunnerSubsets; ++i){
		// Set the grounds index buffer
		d3d11DevCon->IASetIndexBuffer(RunnerIndexBuff, DXGI_FORMAT_R32_UINT, 0);
		// Set the grounds vertex buffer
		d3d11DevCon->IASetVertexBuffers(0, 1, &RunnerVertBuff, &stride, &offset);

		// Set the WVP matrix and send it to the constant buffer in effect file
		WVP = RunnerWorld * camView * camProjection;
		cbPerObj.WVP = XMMatrixTranspose(WVP);
		cbPerObj.World = XMMatrixTranspose(RunnerWorld);
		cbPerObj.difColor = material[RunnerSubsetTexture[i]].difColor;
		cbPerObj.hasTexture = material[RunnerSubsetTexture[i]].hasTexture;
		cbPerObj.hasNormMap = material[RunnerSubsetTexture[i]].hasNormMap;
		d3d11DevCon->UpdateSubresource(cbPerObjectBuffer, 0, NULL, &cbPerObj, 0, 0);
		d3d11DevCon->VSSetConstantBuffers(0, 1, &cbPerObjectBuffer);
		d3d11DevCon->PSSetConstantBuffers(1, 1, &cbPerObjectBuffer);
		if (material[RunnerSubsetTexture[i]].hasTexture)
			d3d11DevCon->PSSetShaderResources(0, 1, &meshSRV[material[RunnerSubsetTexture[i]].texArrayIndex]);
		if (material[RunnerSubsetTexture[i]].hasNormMap)
			d3d11DevCon->PSSetShaderResources(1, 1, &meshSRV[material[RunnerSubsetTexture[i]].normMapTexArrayIndex]);
		d3d11DevCon->PSSetSamplers(0, 1, &CubesTexSamplerState);

		d3d11DevCon->RSSetState(RSCullNone);
		int indexStart = RunnerSubsetIndexStart[i];
		int indexDrawAmount = RunnerSubsetIndexStart[i + 1] - RunnerSubsetIndexStart[i];
		if (!material[RunnerSubsetTexture[i]].transparent)
			d3d11DevCon->DrawIndexed(indexDrawAmount, indexStart, 0);
	}
